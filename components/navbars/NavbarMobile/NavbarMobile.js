    ...
    <>
      ...
      >
        ...
        <button
          ...
          className='flex items-center justify-center h-14 w-14 focus:outline-none'
        >
          <svg
      ...
      <Transition
        ...
        leaveTo={'opacity-0'}
      >
        <div className='fixed inset-0 z-10 flex flex-col w-screen bg-white bg-opacity-75 md:hidden bg-blur-5'>
          {/* CLOSE */}
          <div className='flex justify-end w-full h-14'>
            <button
              onClick={toggleMenu}
              className='flex items-center justify-center text-gray-600 h-14 w-14 focus:outline-none'
            >
              <svg
                className='h-6 fill-current'
            ...
            <div className='flex flex-col text-3xl font-light text-center text-gray-600'>
              {links.map(link => (
                <Link key={link.label} href={link.path}>
                  <a className='px-4 py-2 my-2 transition-colors duration-100 hover:text-gray-900'>
                    {link.label}
                  </a>
                </Link>
              ))}
