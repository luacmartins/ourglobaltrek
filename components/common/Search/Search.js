...
export default function Search({ toggle }) {
   ...
   return (
      <>
         <div onClick={toggle} className="fixed inset-0 z-10 w-screen h-screen bg-gray-300 bg-blur-3 bg-opacity-75"></div>
         <div className="fixed z-20 w-11/12 overflow-hidden bg-white rounded-lg top-1/4 left-1/2 transform -translate-x-1/2 -translate-y-1/4 h-14 md:w-1/2">
            <form onSubmit={handleSubmit} className="flex items-center h-full text-blue-500">
               ...
               <input value={value} onChange={handleChange} className="flex-1 px-4 text-lg text-gray-600 placeholder-gray-500 rounded h-14 focus:outline-none" type="text" placeholder="Search" autoFocus />
            </form>
         </div>
      </>
   )
