...
export default function CardPostList({ data, error, home }) {
  ...
  return (
    <>
      <div
        className={
          `grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 featured justify-center mt-8 mb-8 ` +
          `${home ? "gap-6 md:gap-8" : "gap-6 md:gap-x-8 md:gap-y-16"}`
        }
      >
        {data &&
          data.map((post, i) => (
            <Reveal
              ...
              triggerOnce
              className={home && !i ? "md:col-span-3 lg:col-span-4" : ""}
              delay={delay(i)}
            >
